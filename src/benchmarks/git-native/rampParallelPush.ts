...
export class RampParallelPushBenchmark extends BaseBenchmark {
  ...
  constructor(
    ...
    super(
      namePrefix ? `${namePrefix} / Ramp Parallel Push` : "Ramp Parallel Push",
      config
    );
  ...
  async run(): Promise<BenchmarkResult[]> {
    ...
    const {
      ...
      stopP95Ms,
      stopErrorRate,
    } = this.config.benchmarks.rampParallelPush;
    ...
    for (
      ...
      for (let i = 0; i < totalBranches; i++) {
        ...
        execFileSync(
          "git",
          ["commit", "-m", `Ramp parallel push commit ${branchCounter}`],
          {
            cwd: this.baseRepoPath,
            stdio: "pipe",
          }
      ...
      console.log(
        `  Step metrics: mean=${metrics.latency.mean.toFixed(2)}ms p95=${metrics.latency.p95.toFixed(
          2
        )}ms errorRate=${(errorRate * 100).toFixed(1)}%`
      );
