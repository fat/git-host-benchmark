...
export class CreateCommitBenchmark extends BaseBenchmark {
  ...
  async run(): Promise<BenchmarkResult | BenchmarkResult[]> {
    ...
    const { iterations, fileSizes, filesPerCommit, concurrency } =
      this.config.benchmarks.sdkCreateCommit;
    ...
    for (const fileSize of fileSizes) {
      for (const fileCount of filesPerCommitLevels) {
        ...
        for (const concurrencyLevel of concurrencyLevels) {
          ...
          console.log(
            `\r    ✓ Completed ${timings.length} commits (${fileCount} file(s) of ${fileSize}B each) at concurrency ${concurrencyLevel}`
          );
          if (errors > 0) {
