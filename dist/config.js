...
function loadConfigFile(configPath) {
    try {
        ...
        const expanded = JSON.parse(JSON.stringify(config).replace(/"\$\{([^}]+)\}"/g, (match, varName) => {
...
export function loadConfig(configPath) {
    ...
    const codeStorageEnabled = fileConfig.targets?.codeStorage?.enabled ?? true;
    ...
    if (codeStorageEnabled) {
        try {
            ...
            throw new Error(`Failed to load private key from ${privateKeyFullPath}: ${error instanceof Error ? error.message : error}`);
        }
    }
    // Merge with defaults
    const config = {
        localRepo: fileConfig.localRepo ||
        ...
        targets: {
            ...
            github: {
                ...
                owner: fileConfig.targets?.github?.owner ||
                    process.env.GITHUB_OWNER ||
                    undefined,
                ownerType: fileConfig.targets?.github?.ownerType ||
                    process.env.GITHUB_OWNER_TYPE ||
                    "org",
                repoPrefix: fileConfig.targets?.github?.repoPrefix ||
                    process.env.GITHUB_REPO_PREFIX ||
                    "git-host-benchmark",
                visibility: fileConfig.targets?.github?.visibility ||
                    process.env.GITHUB_VISIBILITY ||
                    "private",
                apiBase: fileConfig.targets?.github?.apiBase ||
        ...
        benchmarks: {
            ...
            rampParallelPush: {
                ...
                minConcurrency: fileConfig.benchmarks?.rampParallelPush?.minConcurrency ?? 1,
                maxConcurrency: fileConfig.benchmarks?.rampParallelPush?.maxConcurrency ?? 10,
            ...
            sdkCreateCommit: {
                ...
                filesPerCommit: fileConfig.benchmarks?.sdkCreateCommit?.filesPerCommit ?? 1,
...
function validateConfig(config) {
    ...
    if (config.benchmarks.worktreeCommit.enabled) {
        validatePositive(config.benchmarks.worktreeCommit.concurrency, "Worktree concurrency");
        validatePositive(config.benchmarks.worktreeCommit.iterations, "Worktree iterations");
    }
    if (config.benchmarks.parallelPush.enabled) {
        validatePositive(config.benchmarks.parallelPush.concurrency, "Parallel push concurrency");
        validatePositive(config.benchmarks.parallelPush.iterations, "Parallel push iterations");
    }
    if (config.benchmarks.rampClone.enabled) {
        validatePositive(config.benchmarks.rampClone.minConcurrency, "Ramp clone min concurrency");
        validatePositive(config.benchmarks.rampClone.maxConcurrency, "Ramp clone max concurrency");
        validatePositive(config.benchmarks.rampClone.step, "Ramp clone step");
        validatePositive(config.benchmarks.rampClone.iterations, "Ramp clone iterations");
        if (config.benchmarks.rampClone.maxConcurrency <
    ...
    if (config.benchmarks.rampParallelPush.enabled) {
        validatePositive(config.benchmarks.rampParallelPush.minConcurrency, "Ramp parallel push min concurrency");
        validatePositive(config.benchmarks.rampParallelPush.maxConcurrency, "Ramp parallel push max concurrency");
        validatePositive(config.benchmarks.rampParallelPush.step, "Ramp parallel push step");
        validatePositive(config.benchmarks.rampParallelPush.iterations, "Ramp parallel push iterations");
        if (config.benchmarks.rampParallelPush.maxConcurrency <
            config.benchmarks.rampParallelPush.minConcurrency) {
            throw new Error("Ramp parallel push maxConcurrency must be >= minConcurrency");
        }
    }
    if (config.benchmarks.sdkCreateCommit.enabled) {
        if (config.benchmarks.sdkCreateCommit.fileSizes.length === 0) {
            throw new Error("At least one file size must be specified for SDK createCommit benchmark");
        }
