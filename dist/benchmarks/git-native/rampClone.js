...
export class RampCloneBenchmark extends BaseBenchmark {
    ...
    constructor(config, remoteUrl, namePrefix) {
        super(namePrefix ? `${namePrefix} / Ramp Clone` : "Ramp Clone", config);
        this.remoteUrl = remoteUrl;
        this.workingDir = config.workingDir;
    }
    async run() {
        console.log(`\n📈 Running ${this.name} benchmark...`);
        const { minConcurrency, maxConcurrency, step, iterations, stopP95Ms, stopErrorRate } = this.config.benchmarks.rampClone;
...
//# sourceMappingURL=rampClone.js.map