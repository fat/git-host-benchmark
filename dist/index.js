...
async function main() {
    ...
    try {
        ...
        if (selectedTargets.length === 0) {
            throw new Error("No targets selected. Enable targets in config or pass --targets.");
        }
        ...
        try {
            ...
            for (const target of selectedTargets) {
                ...
                if (config.benchmarks.initialPush.enabled && shouldRun("initialPush")) {
                    const benchmark = new InitialPushBenchmark(config, target.provider, namePrefix);
                ...
                if (config.benchmarks.worktreeCommit.enabled &&
                    ...
                    const benchmark = new WorktreeCommitBenchmark(config, baseClonePath, namePrefix);
                    const result = await benchmark.run();
                    processResults(result);
                }
                // 4. Parallel Push
                if (config.benchmarks.parallelPush.enabled &&
                    ...
                    const benchmark = new ParallelPushBenchmark(config, remoteUrl, baseClonePath, namePrefix);
                    const result = await benchmark.run();
                    processResults(result);
                }
                // 4b. Ramp Clone
                ...
                if (repo) {
                    // SDK: listFiles
                    if (config.benchmarks.sdkListFiles.enabled &&
                        shouldRun("sdkListFiles")) {
                        const benchmark = new ListFilesBenchmark(config, target.provider, repo, namePrefix);
                        const result = await benchmark.run();
                        processResults(result);
                    }
                    // SDK: createCommit
                    if (config.benchmarks.sdkCreateCommit.enabled &&
                        ...
                        const benchmark = new CreateCommitBenchmark(config, target.provider, repo, namePrefix);
                        const result = await benchmark.run();
                        processResults(result);
                    }
                    // SDK: deletePath
                    if (config.benchmarks.sdkDeletePath.enabled &&
                        shouldRun("sdkDeletePath")) {
                        const benchmark = new DeletePathBenchmark(config, target.provider, repo, namePrefix);
                        const result = await benchmark.run();
                        processResults(result);
                    }
                }
